...
func (r *mongoUserStore) Create(ctx context.Context, user *identra.UserModel) error {
...
func (r *mongoUserStore) GetByID(ctx context.Context, id string) (*identra.UserModel, error) {
	return r.findOne(ctx, bson.M{"_id": id}, "user_id", id)
}

func (r *mongoUserStore) GetByEmail(ctx context.Context, email string) (*identra.UserModel, error) {
	return r.findOne(ctx, bson.M{"email": email}, "email", email)
}

func (r *mongoUserStore) Update(ctx context.Context, user *identra.UserModel) error {
...
func (r *mongoUserStore) List(ctx context.Context, offset, limit int) ([]*identra.UserModel, error) {
	opts := options.Find()
	if offset > 0 {
	...
	users := make([]*identra.UserModel, 0, len(records))
...
func (r *mongoUserStore) findOne(ctx context.Context, filter bson.M, key string, value any) (*identra.UserModel, error) {
	var record userRecord
