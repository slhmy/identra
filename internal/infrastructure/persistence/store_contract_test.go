...
func runUserStoreContract(t *testing.T, factory storeFactory) {
	...
	user := &domain.UserModel{Email: "user@example.com"}
	...
	duplicate := &domain.UserModel{Email: user.Email}
	if err := userStore.Create(ctx, duplicate); !errors.Is(err, domain.ErrAlreadyExists) {
		t.Fatalf("duplicate email error = %v, want %v", err, domain.ErrAlreadyExists)
	}
	...
	second := &domain.UserModel{Email: "second@example.com"}
	...
	if _, err := userStore.GetByID(ctx, user.ID); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("get deleted user error = %v, want %v", err, domain.ErrNotFound)
	}
	if err := userStore.Delete(ctx, user.ID); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("delete missing user error = %v, want %v", err, domain.ErrNotFound)
	}
}
...
func runExternalIdentityStoreContract(t *testing.T, factory storeFactory) {
	...
	user := &domain.UserModel{Email: "oauth-user@example.com"}
	if err := userStore.Create(ctx, user); err != nil {
		t.Fatalf("create user: %v", err)
	}
	...
	duplicate := &domain.ExternalIdentityModel{
		UserID:         user.ID,
		Provider:       identity.Provider,
		ProviderUserID: identity.ProviderUserID,
	}
	if err := identityStore.Create(ctx, duplicate); !errors.Is(err, domain.ErrAlreadyExists) {
		t.Fatalf("duplicate provider identity error = %v, want %v", err, domain.ErrAlreadyExists)
	}
	...
	second := &domain.ExternalIdentityModel{
		UserID:         user.ID,
		Provider:       "google",
	...
	if _, err := identityStore.GetByProviderID(ctx, "github", "missing"); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("get missing identity error = %v, want %v", err, domain.ErrNotFound)
	}
}
